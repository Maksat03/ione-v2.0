...
function retake() {
  ...
  window.location.hash = 'scroll-test';
...
const sendData = () => {
  if (test_was_passed) {
    ...
    var formData = {
      csrfmiddlewaretoken: $('.check_answers input[name="csrfmiddlewaretoken"]').val(),
    };
    ...
    $.ajax({
      ...
      success: (response) => {
        document.querySelector("#scroll-test").scrollIntoView({
          behavior: "smooth",
          block: "start"
        })
        ...
        document.getElementsByClassName("choose_right")[0].style.display = "none";
...
$(".check_answers").submit(function (event) {
